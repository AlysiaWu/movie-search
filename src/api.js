...
const _getPopularMovies = (pg = 1) => axios.get(`${APIURL}/movie/popular?api_key=${process.env.REACT_APP_APIKEY}&language=en-US&page=${pg}`);
...
const _showMovieById = id => axios.get(`${APIURL}/movie/${id}?api_key=${process.env.REACT_APP_APIKEY}&language=en-US`);
...
const _showActorById = id =>  axios.get(`${APIURL}/person/${id}?api_key=${process.env.REACT_APP_APIKEY}&language=en-US`);
...
const _getAPIConfig = async () => {
    const res = await axios.get(`${APIURL}/configuration?api_key=${process.env.REACT_APP_APIKEY}`)
